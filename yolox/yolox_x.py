...
from what.models.detection.yolox.exp import get_exp
...
class YOLOX_X:
    ...
    def predict(self, image):
        ...
        if outputs[0] is not None:
            boxes  = outputs[0][:, 0:4].cpu().detach().numpy()
            class_ids = outputs[0][:, -1].cpu().detach().numpy().astype(np.uint32)
            probs = (outputs[0][:, 4] * outputs[0][:, 5]).cpu().detach().numpy()
